...
class CSInfo_Template {
    var $views_path;
   	var $o; // options

   	function __construct($o = array()){
   		$this->views_path = CSINFO_DIR . '/views';
   		$this->o = $o;
   	}
	...
	function get_view($view, $data=array()){
		$view = $this->_validate_view_name($view);
   		$target_fn = $this->_get_view_fn($view);
		...
		extract($data, EXTR_OVERWRITE);
   		...
   		if(file_exists($target_fn)):
   			...
   			include ($this->_get_view_fn('error/view_does_not_exist'));
   		endif;
   		$view = ob_get_clean();
	 ...
	 * @return string
	 */
	private function _get_view_fn($view){
	 ...
	 * @return string
	 */
	private function _validate_view_name($view){
